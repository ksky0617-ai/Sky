...
> ## ⚠️ 일부 권고가 대체되었다 (2026-08-15)
>
> 이 문서의 **조사 결과(§1–§9)는 전부 유효하다.** 그러나 두 개의 **권고**는 이후 지시서에 의해 대체되었다. 감사 기록으로서의 가치를 위해 원문은 그대로 두고, 해당 위치에 표시했다.
>
> | 위치 | 원래 권고 | 대체 |
> | --- | --- | --- |
> | **§15.4** | 커머스 백엔드로 Shopify Headless | **[ADR-004](../adr/ADR-004-commerce-hosting-architecture.md)** — Cloudflare Pages + Stripe hosted checkout ($0 고정비) |
> | **§19** | MVP-0 (커머스 없는 브랜드 월드) → MVP-1 | **[ADR-002](../adr/ADR-002-mvp-boundary.md)** — MVP-1P: 옷 한 벌, 완결된 루프 |
>
> 또한 §15.3에서 참조한 D1 §60 "재고 0"은 **[ADR-003](../adr/ADR-003-inventory-model.md)**으로 정정되었다 — 목표는 숫자 0이 아니라 판매 전 자본 위험의 최소화이며, print-on-demand는 브랜드 근거로 실격이다.
...
> **🛑 이 권고는 [ADR-004](../adr/ADR-004-commerce-hosting-architecture.md)로 대체되었다.** 아래 판단은 비용 제약이 없는 상태에서 내려진 것이며, D2 §92의 near-$0 제약 하에서는 유지되지 않는다. 상품도 매출도 없는 단계에서 Shopify는 첫 판매 전에 약 $468을 청구한다. 현재 결정은 **Cloudflare Pages + Stripe hosted checkout ($0 고정비)**이다. 원문은 판단 근거의 기록으로 남긴다.
...
> **🛑 아래 MVP 구분은 [ADR-002](../adr/ADR-002-mvp-boundary.md)로 대체되었다.** MVP-0(커머스 없는 브랜드 월드 선출시)은 D3 §58 금지 1이 지목하는 실패 모드다 — 예쁜 UI를 먼저 완성하고 사업 모델을 나중에 생각하는 것. 전제("상품이 없으니 커머스 화면은 껍데기가 된다")는 참이었으나 결론이 틀렸다. 현재 목표는 **MVP-1P: 옷 한 벌을 끝까지 파는 것**이다. 아래 §19.3(Atlas 최소 12행)은 여전히 유효하다.
