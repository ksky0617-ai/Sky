...
**Updated:** 2026-08-15 · **Commit at write time:** `eba4f9d` · **Branch:** `claude/olibana-project-spec-76ivk7`
...
| `CURRENT_CYCLE` | Cycle 5 under governance vNext |
| `CURRENT_CRITICAL_PATH` | ~~품목 결정~~ ✅ → **실측·사양 기입 → 견적 발송** → 원가 → 가격 → minimum_quantity → pre-order |
| `LAST_VERIFIED_STATE` | **V1 unit + V4 mutation, 2026-08-15.** `npm test` → **39/39 pass**. **10 deliberate mutations across 3 modules, all caught**; sources restored byte-identical. Verified: order transitions (20 edges, 3 terminal, guards, idempotency), identifiers (ULID sortability, UUIDv4 non-enumerability, SKU/order-number round-trip), break-even (UNKNOWN never treated as zero, fixed-cost amortisation, MOQ binding). |
| `OPEN_BLOCKERS` | **6 × P0** (P0-1 closed by ADR-005), 7 × P1 — see [`RISK_REGISTER.md`](./RISK_REGISTER.md) and §4 below |
| `RECENT_CHANGE` | **Cycle 5:** `src/identity/ids.ts`, `src/economics/break-even.ts` + tests. Cycle 4: first code — order state machine, ADR-007. Cycle 3: ADR-005/006, `05_OUTERWEAR_SPEC_PACK.md`, HG-R1. Cycle 2: `04_…`. Cycle 1: `PROTOCOL_LOCK.md`, `STATE.md`. Zero runtime dependencies throughout. |
| `RECENT_VERIFICATION` | V-2026-08-15-005 (V1+V4) — identity + economics, 39/39 pass, 6/6 new mutations caught. Prior: V-004, V-003, V-002, V-001. |
...
| `NEXT_ACTION_2` | **Autonomous:** order aggregate binding transitions to identifiers + an append-only transition log (still pure, no persistence) |
...
| **What is verified?** | **Three pure modules at V1 + V4:** order transitions (20 edges, 3 terminal, guards, idempotency), identifiers (ULID time-sortable, customer UUIDv4 non-enumerable, SKU/order-number round-trip), break-even (UNKNOWN never zero, amortisation, MOQ binding). 39/39 pass; 10/10 mutations caught. Everything else remains V0 or unverified. |
...
| BG-05 | Invariant Protection | **PARTIAL** | Mutation-verified: order transitions, SKU/identifier format integrity, UNKNOWN-never-zero in costing. Not yet protected: availability integrity, price/order consistency across persistence. |
...
| **Tests** | `test/order/`, `test/identity/`, `test/economics/` — 39 assertions |
