...
**Updated:** 2026-08-22 · **Commit at write time:** `bfede0d` · **Branch:** `claude/olibana-project-spec-76ivk7`
...
| `LAST_VERIFICATION` | V-2026-08-22-012 — see `VERIFICATION_LOG.md` |
...
| `NEXT_ACTION_2` (`NEXT_ACTIONS`) | **Autonomous:** the HTTP layer — `POST /checkout` and the gateway webhook. `beginCheckout`/`completeCheckout` are implemented and verified; nothing yet routes a request into them, so the form posts to an endpoint that does not exist. |
...
| **What is not verified?** | Payment execution and shipment — `SPEC_ONLY`. Nothing routes HTTP into the checkout code yet, so the form on the product page posts to an endpoint that does not exist: **the selection-to-order path is implemented and tested but not yet reachable by a customer.** Concurrency and crash-during-write **are now tested with spawned processes** (V-2026-08-15-010): both were confirmed as real defects and fixed. Still unverified: **power-loss durability** — nothing calls `fsync`, so an acknowledged record can sit in the OS page cache when the machine loses power. The write lock is filesystem-scoped and would not hold across machines. For the website: screen readers, colour contrast (the palette under test is the construction palette), browsers other than Chromium, Core Web Vitals on a real network. Conformance verifies transcription fidelity, **not that the specification is itself correct**; 4 prose-quantified forbidden rules sit outside machine verification and are disclosed as such. |
...
| BG-04 | Critical Path Coverage | **FALSE** | Website renders; products are catalogued and render from data; a pre-order run decides production from counted commitments and states its window on the page; a customer can select a variant on the page, and the checkout boundary places, prices, snapshots and pays an order. Still absent: **the HTTP layer that connects the form to that code**, a configured payment gateway, shipment, analytics. Cart is refused by ADR-009, not missing. |
...
```
LATEST:      V-2026-08-22-012 (V1 + V2 + V3 + V4 + V5, checkout boundary and visual audit)
COMMAND:     npm run verify   (build + suite; counts deliberately not restated here)
MUTATIONS:   63 run to date · 61 caught · 2 standing survivors, both root-caused and disclosed
...
| **Source** | `src/order/`, `src/catalog/`, `src/preorder/`, `src/checkout/`, `src/persistence/`, `src/identity/`, `src/economics/`, `src/site/` |
| **Tests** | `test/order/`, `test/catalog/`, `test/preorder/`, `test/checkout/`, `test/persistence/`, `test/identity/`, `test/economics/`, `test/conformance/`, `test/site/` — run `npm test` for the count |
