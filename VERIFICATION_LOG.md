...
## V-2026-08-24-033 · V0 + V1 + V3 + V4 · Visual depth and responsive rhythm

| | |
| --- | --- |
| **Target** | `src/site/styles.ts`, `src/site/routes.ts`, `scripts/visual-check.mjs`, `test/site/build.test.ts` |
| **Every page carried two widths** | Prose capped at `--measure` (~630px); tables, preformatted blocks, index lists and figures took the full 1280px container. So a desktop reader saw a paragraph end halfway across the page and directly beneath it a two-column table stretched to twice that, its columns flung apart to fill space nothing else used. **Both rules were correct in isolation** — there is no markup and no unit test in which the conflict exists. It is only visible when the two rendered widths are put beside each other, which is what a screenshot does. Capping `main` at the reading measure makes the page one column; wide content scrolls inside `.table-scroll`, which is what that container was built for and what the accessibility page already claims happens. |
| **A cap that had been dead since it was written** | The reserved 4:5 photography slot declared `max-width: 32rem`, and `figure.media` — the same specificity, **later in the file** — set the full reading measure and won. The slot rendered 630x790: the largest element on the product page, given to the one thing that does not exist, and reading as a failed image rather than as held space. It is now capped at `--space-14`, unfilled, with the sentence explaining the absence inside the frame instead of below it in small grey. |
| **Hierarchy without a seventh type size** | The opening — the h1 and the sentence saying what the page is for — was typographically identical to body copy. It is now closed with a hairline rule, scoped by `:first-of-type` so a section intro further down does not get one. §5 asks for *"hierarchy through space, not through scale variety"* and this project already carries a written exception for its type-size count, so buying the intro's rank with another size would have spent the wrong budget. **No page gained a type size: 6/5/4/3 before, 6/5/4/3 after.** |
| **The Atlas opened on an inventory** | The title sat directly on five bullets — `Focus of Study` is a list, so the one page type with no opening sentence was the one describing a body of research. The sentence is now the Atlas's **own description**: already the entry on `/nature`, already the entry on the home page, already the page's meta description, and shown nowhere on the page it describes. The list moved under a heading naming it, *What is studied*, which also restores the order every index promises the Atlas in. |
| **Mobile rhythm** | The opening had 40px above the title and 32px below it, which on a 390px screen put the first paragraph of body copy against the fold. One step each way. Section spacing drops from 96px to 56px below 640px: a desktop rhythm on a narrow screen reads as the page having ended. |
| **Measured effect — every route gained space, none lost any** | Empty share rose on **60 of 78 renders** and fell on none: atlas/desktop 51%→75%, philosophy/desktop 65%→72%, nature/mobile 57%→67%, design-language/mobile 51%→57%, accessibility/desktop 69%→76%. |
| **Philosophy mobile: 42%, unchanged, and reported as such** | The floor is 40% and the margin is still two points. The spacing changes moved the fold's content down without changing what is on it: Philosophy's first screen is the Project Vision text, and the only way to raise that number further is to publish less of it. That is a content decision, not a spacing one, and it is not being made by adjusting a threshold. |
| **Mutation — 7 planted, 7 correct outcomes** | M11 the one-column cap is removed → **SURVIVED**. M12 the photography slot's cap is removed → killed (642px over a 320px cap). M13 the opening rule is removed → killed. M14 the slot's ratio becomes 1:1 → killed. M15 the Atlas opening is dropped → killed. M16 the Atlas opening becomes hand-written copy → killed. M17 *What is studied* is dropped → killed. |
| **What M11's survival exposed — my own checker, again** | The width check compared each element against **main's own box**. Remove the cap and main grows too, so nothing exceeds its parent and the check passes on exactly the layout it exists to reject. It now compares against the widest paragraph or list actually laid out — the writing has to be one side of the comparison, because the conflict is between the width the writing is set to and the width everything else takes. Re-run under M11 it reports 10 elements per page. A second defect in the same check: it exempted scroll containers in a comment and not in code, and reported the 532px `code` inside the Atlas field-record template — held correctly inside a `pre` that scrolls — as a page-level overflow. |
| **A threshold that measured the wrong thing** | The slot check first failed on mobile at 75% of the column. That was the check being wrong: on a 358px column a photograph legitimately spans most of the width, and a share threshold derived from desktop measures something else there. It now asserts the cap the stylesheet declares, in pixels, read from the token itself. |
| **Executed** | 466/466 tests (from 465). **78 browser renders** — 13 routes × 3 viewports × 2 colour schemes — exit 0, no overflow, no undersized targets, no contrast failures, purchase button legible. Deployment auditor: 17 passed, 0 failed, 1 unverified (no product published, so no checkout exists to write an intent through) — exit 3, unchanged. |
| **Motion budget** | **L1 12 → L2 7 → L3 1.** Descending. The Atlas leaf rose from 7 to 8 with its new section heading, still under L1's 12. No motion was added, no mode changed, no threshold moved. |
| **Accessibility re-verified after the visual change** | Contrast (every element that paints text, against the background actually behind it, both colour schemes, three widths): 0 failures. Target size against the 24px floor: 0 failures. `aria-current`, breadcrumb semantics and one h1 per page: unchanged. |
| **Gates unchanged** | GATE-004 — **0 Atlas rows**, BLOCKED_EXTERNAL. GATE-005 — **0 product images**, BLOCKED_EXTERNAL; the 4:5 slot still holds no picture, and making it read as intentional held space is not a photograph. **COMPLETION remains FALSE.** |
| **Commit** | written against `33b653d` |
